@extends('layouts.app')
@section('title', 'Edit Proof — ' . $label->name)
@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Proof</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <form method="POST" action="{{ route('labels.proof.update', [$label, $proof]) }}" enctype="multipart/form-data" novalidate>
            @csrf
            <input type="hidden" name="_method" value="PUT">
            @include('labels.modules._proof_form', ['proof' => $proof])
            <div class="flex items-center gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                <button type="submit" class="px-6 py-2 text-sm font-medium bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition-colors">Update</button>
