        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h1 class="mt-2 text-2xl font-bold text-gray-900 dark:text-white">Edit Role: {{ $role->name }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Update role information and permissions</p>
                </div>
                                    ...
                                    <nav class="-mb-px flex space-x-4 overflow-x-auto" aria-label="Tabs">
                                        @foreach($permissions as $group => $groupPermissions)
                                            <button type="button"
                                                    ...
                                                    class="permission-tab whitespace-nowrap py-2 px-4 border-b-2 font-medium text-sm {{ $loop->first ? 'active border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300' }}">
                                                {{ $group ?? 'General' }}
                                                    ...
                                                    @foreach($groupPermissions as $permission)
                                                        <label class="flex items-start space-x-3 p-3 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors">
                                                            <input type="checkbox" 
                                                                   name="permissions[]" 
                                                                   value="{{ $permission->id }}"
                                                                   {{ (old('permissions') && in_array($permission->id, old('permissions'))) || (empty(old('permissions')) && $role->permissions->contains($permission->id)) ? 'checked' : '' }}
