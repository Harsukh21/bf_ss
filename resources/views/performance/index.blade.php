...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            @if(!empty($rateTableStats['summary']) && ($rateTableStats['summary']['total_tables'] ?? 0) > 0)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md lg:col-span-2">
                    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <div>
                            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Market Rate Tables Overview</h2>
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Top dynamic tables that can impact query latency in live.</p>
                        </div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">
                            Snapshot cached for 60s
                        </div>
                    </div>
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div class="bg-gray-50 dark:bg-gray-900/20 rounded-lg p-4">
                                <p class="text-sm text-gray-500 dark:text-gray-400">Dynamic Tables</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ number_format($rateTableStats['summary']['total_tables']) }}</p>
                            </div>
                            <div class="bg-gray-50 dark:bg-gray-900/20 rounded-lg p-4">
                                <p class="text-sm text-gray-500 dark:text-gray-400">Total Size</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $rateTableStats['summary']['total_size'] }}</p>
                            </div>
                            <div class="bg-gray-50 dark:bg-gray-900/20 rounded-lg p-4">
                                <p class="text-sm text-gray-500 dark:text-gray-400">Estimated Rows</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ number_format($rateTableStats['summary']['total_rows']) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-900/40">
                                    <tr>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Table</th>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Rows (est.)</th>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Size</th>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Last Vacuum</th>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Last Auto-Vacuum</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    @forelse($rateTableStats['top_tables'] as $table)
                                        <tr>
                                            <td class="px-4 py-2 text-sm font-medium text-gray-900 dark:text-gray-100">{{ $table['name'] }}</td>
                                            <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">{{ number_format($table['rows']) }}</td>
                                            <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">{{ $table['size'] }}</td>
                                            <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">{{ $table['last_vacuum'] ?? 'N/A' }}</td>
                                            <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">{{ $table['last_autovacuum'] ?? 'N/A' }}</td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-4 py-4 text-center text-sm text-gray-500 dark:text-gray-400">No market rate tables detected.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
